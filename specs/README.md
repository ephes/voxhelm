...
- Remaining planned work is narrower now: an async Wagtail transcript-completion follow-on for `python-podcast` / `django-cast`, a large-media batch-input follow-on so consumers do not need their own chunking/transcoding logic, operational work beyond the current `whisper.cpp` + `mlx-whisper` STT set plus the experimental non-default WhisperKit path, Archive article-audio consumer follow-on, and M4/OpenClaw.
