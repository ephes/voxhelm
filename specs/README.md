...
- M1a is implemented and deployed.
- The live M1a shape is a single Django + `uvicorn` process on `studio` with private HTTPS ingress on `macmini` at `https://voxhelm.home.xn--wersdrfer-47a.de`.
- Implemented endpoints: `GET /v1/health` and `POST /v1/audio/transcriptions`.
...
- Archive has been validated against the deployed service using env vars only.
- M1b+ work (batch jobs, Django Tasks runtime, MinIO, Wyoming, TTS, extra consumers) remains planned, not implemented.
