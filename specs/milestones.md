...
- async Wagtail transcript-completion follow-on for `python-podcast` / `django-cast`: editor-triggered transcript generation is now non-blocking, background completion runs through consumer-local Django Tasks, editors see queued/running/succeeded/failed status on Episode and Audio admin surfaces, and the Audio edit-view transcript action returns cleanly via its own POST form
...
| Follow-on | Async Wagtail transcript completion | python-podcast / django-cast editors | M1c, C18 | delivered on 2026-03-14 via consumer-local Django Tasks + persisted editor-visible status |
